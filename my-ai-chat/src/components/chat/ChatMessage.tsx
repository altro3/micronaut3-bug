...
export default function ChatMessage({ message }: ChatMessageProps) {
    ...
    return (
        <div className={`flex gap-4 max-w-3xl ${isUser ? 'ml-auto flex-row-reverse' : 'mr-auto'}`}>
            ...
            {/* Тело сообщения */}
            {/* ДОБАВЛЕНО: whitespace-pre-wrap на родительском уровне */}
            <div className="space-y-2 max-w-full overflow-hidden whitespace-pre-wrap">
                <div className={`p-4 rounded-xl text-sm leading-relaxed ${isUser ? 'bg-indigo-500/10 border border-indigo-500/20 text-indigo-100' : 'bg-gray-800 border border-gray-700 text-gray-200'}`}>
                    {isUser ? (
                        ...
                        <ReactMarkdown
                            ...
                            components={{
                                p: ({ ...props }) => <p className="mb-3 last:mb-0 text-gray-300 leading-relaxed" {...props} />,
                                strong: ({ ...props }) => <strong className="font-bold text-indigo-400" {...props} />,
                                ul: ({ ...props }) => <ul className="list-disc pl-5 my-2 space-y-1" {...props} />,
                                ol: ({ ...props }) => <ol className="list-decimal pl-5 my-2 space-y-1" {...props} />,
                                li: ({ ...props }) => <li className="text-gray-300 leading-normal" {...props} />,
                                code: ({ ...props }) => <code className="bg-gray-950 px-1.5 py-0.5 rounded text-amber-400 font-mono text-xs" {...props} />,
                                h1: ({ ...props }) => <h1 className="text-xl font-bold text-gray-100 mt-4 mb-2" {...props} />,
                                h2: ({ ...props }) => <h2 className="text-lg font-bold text-gray-100 mt-4 mb-2" {...props} />,
