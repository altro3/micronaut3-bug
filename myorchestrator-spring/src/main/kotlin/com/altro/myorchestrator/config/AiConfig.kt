...
class AiConfig {
    ...
    @Bean
    fun consultantAgentClient(chatClientBuilder: ChatClient.Builder, chatMemoryRepository: ChatMemoryRepository): ChatClient {
        return chatClientBuilder
            .defaultAdvisors(chatMemoryAdvisor(chatMemoryRepository))
            .defaultSystem(
                """
    ...
    fun yandexAgentClient(
        ...
        return chatClientBuilder
            ...
            .defaultSystem(
                """
            Ты — ИИ-Агент Яндекс.Директ. Твоя цель — собрать все необходимые параметры и настроить рекламную кампанию в Яндексе, используя доступные инструменты бэкенда.
            Ты ведешь живой диалог, извлекаешь параметры и управляешь состоянием системы.
            ...
            🎯 ТЕХНИЧЕСКИЙ КОНТЕКСТ СЕССИИ И ПРАВИЛА МОДЕРАЦИИ (КВАДРАНТ):
            {moderationRules}
        """.trimIndent()
            )
            .build()
    ...
    @Bean
    fun vkAgentClient(chatClientBuilder: ChatClient.Builder, chatMemoryRepository: ChatMemoryRepository) =
        chatClientBuilder
            .defaultAdvisors(chatMemoryAdvisor(chatMemoryRepository))
            .defaultSystem(
                """
