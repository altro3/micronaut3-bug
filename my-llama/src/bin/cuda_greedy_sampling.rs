...
unsafe extern "C" {
    pub fn launch_greedy_sampling(token_id: *mut i32, logits: *const f32, vocab_size: i32, threads_per_block: i32, stream: *mut c_void);
...
fn main() {
    println!("=== УЛЬТИМАТИВНЫЙ СТРЕСС-БЕНЧМАРК И ВАЛИДАЦИЯ GREEDY SAMPLING ===");

    let vocab_size = 152064;
    let threads_per_block = 256;

    println!("Параметры жадного сэмплинга (ArgMax):");
    println!("Vocab Size: {}, Threads: {}", vocab_size, threads_per_block);
    ...
    unsafe {
        ...
        for _ in 0..NUM_WARMUP {
            launch_greedy_sampling(
                d_token_id.ptr as *mut i32,
                d_logits.ptr as *const f32,
                vocab_size,
                threads_per_block,
                stream,
            );
        ...
        println!("Запуск телеметрии... Вычисляем чистую задержку ArgMax на Blackwell.");
        ...
        let bytes_processed = ((vocab_size * 4) + 4) as f64;
        let avg_bandwidth_gbps = (bytes_processed / 1e9) / (avg_lat / 1e6);

        println!("\n📊 === РЕЗУЛЬТАТЫ СТАТИСТИЧЕСКОГО АНАЛИЗА GREEDY SAMPLING ===");
        ...
        println!("Эффективная скорость чтения VRAM:             {:.2} ГБ/сек", avg_bandwidth_gbps);
        println!("-------------------------------------------------------");
        ...
        if h_token_id[0] == target_token_idx as i32 {
            println!("\n🎉 ПОБЕДА! Легковесное ядро жадного выбора идеально определило ArgMax токен!");
        } else {
