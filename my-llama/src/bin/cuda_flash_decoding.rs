...
fn main() {
    println!("=== УЛЬТИМАТИВНЫЙ СТРЕСС-БЕНЧМАРК И ВАЛИДАЦИЯ FLASH DECODING ===");
    ...
    let chunk_size = 256;
    let num_chunks = (current_seq_len + chunk_size - 1) / chunk_size;

    println!("Боевая геометрия внимания Qwen-35B (GQA):");
    println!(
        "Heads: {}, KV Heads: {}, Dim: {}, Context: {}, Chunk Size: {} (Total Chunks: {})",
    ...
    let partial_out_size = (num_heads * num_chunks * head_dim) as usize;
    let partial_meta_size = (num_heads * num_chunks) as usize;
    ...
    unsafe {
        ...
        println!("Запуск телеметрии... Забиваем асинхронную очередь GPU двухфазным графом.");
        ...
        let static_bytes = (q_size + kv_cache_size * 2 + out_size) * 4;
        let partial_bytes = (partial_out_size * 2 + partial_meta_size * 4) * 4;
        let bytes_processed = (static_bytes + partial_bytes) as u64;
        ...
        println!("\n📊 === РЕЗУЛЬТАТЫ ГЛУБОКОГО СТАТИСТИЧЕСКОГО АНАЛИЗА FLASH DECODING ===");
        ...
        println!("\n--- ЧЕСТНАЯ МАТЕМАТИЧЕСКАЯ ВАЛИДАЦИЯ FLASH DECODING ---");
        ...
        if error < 1e-3 {
            println!("\n🚀 ПОБЕДА! Двухфазный блочный граф Flash-Decoding выдает стопроцентную точность!");
        } else {
