...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut model_graph = match ModelFactory::create_from_config(
        config_path,
        weights_path,
        batch_size,
        is_training,
    ...
    println!(
        "[INIT] Граф успешно скомпилирован и загружен в VRAM за {:.2?}",
        timer.elapsed()
    );
    ...
    for _ in 0..100 {
        ...
        logits_gpu_buffer.copy_to_host_async(&mut host_logits, &stream);
    ...
    println!("\n\n[ГЕНЕРАЦИЯ ЗАВЕРШЕНА]");
    println!(
        "|-> Всего сгенерировано токенов: {}",
        generated_tokens_count
    );
    println!("|-> Чистое время генерации: {:.2?}", elapsed);
    println!(
        "|-> Промышленная скорость инференса: {:.2} токенов/сек (t/s)",
        tps
    );
