#include <cuda_runtime.h>
#include <device_launch_parameters.h>
#include <cuda_fp16.h>
#include <cuda_fp8.h>
#include <math.h>
#include <stdint.h>

#include "data_types.h"

#define BLOCK_M 64
#define BLOCK_N 64

extern "C" {
void launch_flash_attention_prefill_fp16(
    half *output, const half *query, const half *key, const half *value,
    int batch_size, int q_seq_len, int kv_seq_len, int num_heads, int num_kv_heads,
    int head_dim, int threads_per_block, float scale, size_t shared_mem_size, cudaStream_t stream
);

void launch_flash_attention_prefill_fp8(
    __nv_fp8_e4m3 *output, const __nv_fp8_e4m3 *query, const __nv_fp8_e4m3 *key, const __nv_fp8_e4m3 *value,
    int batch_size, int q_seq_len, int kv_seq_len, int num_heads, int num_kv_heads,
    int head_dim, int threads_per_block, float scale, size_t shared_mem_size, cudaStream_t stream
);
}

template<int HEAD_DIM>
__global__ void flash_attention_scalar_prefill_kernel(
    float* __restrict__ output,
    const float* __restrict__ query,
    const float* __restrict__ key,
    const float* __restrict__ value,
    const int q_seq_len,
    const int kv_seq_len,
    const int num_heads,
    const int num_kv_heads,
    const float scale
) {
    const int q_head_idx = blockIdx.x;
    const int tile_m_idx = blockIdx.y;

    const int tid = threadIdx.x;
    const int warp_id = tid / 32;
    const int lane_id = tid % 32;
    const int num_warps = blockDim.x / 32;

    const int kv_head_ratio = num_heads / num_kv_heads;
    const int kv_head_idx = q_head_idx / kv_head_ratio;

    extern __shared__ uint8_t s_dynamic_buffer[];

    float* s_q = reinterpret_cast<float*>(s_dynamic_buffer);
    float* s_k = s_q + BLOCK_M * HEAD_DIM;
    float* s_v = s_k + BLOCK_N * HEAD_DIM;

    float* s_row_m = s_v + BLOCK_N * HEAD_DIM;
    float* s_row_l = s_row_m + BLOCK_M;
    float* s_acc_o = s_row_l + BLOCK_M;

    for (int m = tid; m < BLOCK_M; ++m) {
        s_row_m[m] = -1e20f;
        s_row_l[m] = 0.0f;
        for (int d = 0; d < HEAD_DIM; ++d) {
            s_acc_o[m * HEAD_DIM + d] = 0.0f;
        }
    }
    __syncthreads();

    const int start_m = tile_m_idx * BLOCK_M;
    const int q_stride = num_heads * HEAD_DIM;
    const int kv_stride = num_kv_heads * HEAD_DIM;

    for (int i = tid; i < BLOCK_M * HEAD_DIM; i += blockDim.x) {
        const int m = i / HEAD_DIM;
        const int d = i % HEAD_DIM;
        if (start_m + m < q_seq_len) {
            s_q[m * HEAD_DIM + d] = query[(start_m + m) * q_stride + q_head_idx * HEAD_DIM + d];
        } else {
            s_q[m * HEAD_DIM + d] = 0.0f;
        }
    }
    __syncthreads();

    const int num_n_tiles = (kv_seq_len + BLOCK_N - 1) / BLOCK_N;

    for (int tile_n_idx = 0; tile_n_idx < num_n_tiles; ++tile_n_idx) {
        const int start_n = tile_n_idx * BLOCK_N;

        for (int i = tid; i < BLOCK_N * HEAD_DIM; i += blockDim.x) {
            const int n = i / HEAD_DIM;
            const int d = i % HEAD_DIM;
            if (start_n + n < kv_seq_len) {
                s_k[n * HEAD_DIM + d] = key[(start_n + n) * kv_stride + kv_head_idx * HEAD_DIM + d];
                s_v[n * HEAD_DIM + d] = value[(start_n + n) * kv_stride + kv_head_idx * HEAD_DIM + d];
            } else {
                s_k[n * HEAD_DIM + d] = 0.0f;
                s_v[n * HEAD_DIM + d] = 0.0f;
            }
        }
        __syncthreads();

        for (int m = warp_id; m < BLOCK_M; m += num_warps) {
            if (start_m + m >= q_seq_len) continue;

            float scores[BLOCK_N];
            float local_tile_max = -1e20f;

            for (int n = 0; n < BLOCK_N; ++n) {
                if (start_n + n >= kv_seq_len || (start_m + m) < (start_n + n)) {
                    scores[n] = -1e20f;
                } else {
                    float sum = 0.0f;
                    for (int d = 0; d < HEAD_DIM; ++d) {
                        sum += s_q[m * HEAD_DIM + d] * s_k[n * HEAD_DIM + d];
                    }
                    scores[n] = sum * scale;
                    local_tile_max = fmaxf(local_tile_max, scores[n]);
                }
            }

            const float old_m = s_row_m[m];
            const float new_m = fmaxf(old_m, local_tile_max);

            float p_sum = 0.0f;
            for (int n = 0; n < BLOCK_N; ++n) {
                if (scores[n] > -1e19f) {
                    scores[n] = expf(scores[n] - new_m);
                    p_sum += scores[n];
                } else {
                    scores[n] = 0.0f;
                }
            }

            const float scale_old = expf(old_m - new_m);
            s_row_l[m] = s_row_l[m] * scale_old + p_sum;
            s_row_m[m] = new_m;

            for (int d = lane_id; d < HEAD_DIM; d += 32) {
                float v_sum = 0.0f;
                for (int n = 0; n < BLOCK_N; ++n) {
                    v_sum += scores[n] * s_v[n * HEAD_DIM + d];
                }
                s_acc_o[m * HEAD_DIM + d] = s_acc_o[m * HEAD_DIM + d] * scale_old + v_sum;
            }
        }
        __syncthreads();
    }

    for (int m = warp_id; m < BLOCK_M; m += num_warps) {
        if (start_m + m < q_seq_len) {
            const float inv_l = 1.0f / (s_row_l[m] + 1e-9f);
            for (int d = lane_id; d < HEAD_DIM; d += 32) {
                output[(start_m + m) * q_stride + q_head_idx * HEAD_DIM + d] = s_acc_o[m * HEAD_DIM + d] * inv_l;
            }
        }
    }
}

extern "C" {
void launch_flash_attention_prefill(
    void *output,
    const void *query,
    const void *key,
    const void *value,
    const int data_type_id,
    const int batch_size,
    const int q_seq_len,
    const int kv_seq_len,
    const int num_heads,
    const int num_kv_heads,
    const int head_dim,
    const int threads_per_block,
    void *stream_ptr
) {
    if (q_seq_len == 0 || kv_seq_len == 0) return;

    const float scale = 1.0f / sqrtf(static_cast<float>(head_dim));
    const auto stream = static_cast<cudaStream_t>(stream_ptr);

    const int num_m_tiles = (q_seq_len + BLOCK_M - 1) / BLOCK_M;
    dim3 grid(num_heads * batch_size, num_m_tiles);

    size_t sizeof_elem = 2;
    if (data_type_id == static_cast<int>(DataType::FP32)) {
        sizeof_elem = 4;
    } else if (data_type_id == static_cast<int>(DataType::FP8)) {
        sizeof_elem = 1;
    }

    const size_t shared_mem_size = (BLOCK_M * head_dim + 2 * BLOCK_N * head_dim) * sizeof_elem
                                   + (2 * BLOCK_M) * sizeof(float)
                                   + (BLOCK_M * head_dim) * sizeof(float)
                                   + (BLOCK_M * BLOCK_N) * sizeof(float)
                                   + (BLOCK_M * BLOCK_N) * sizeof_elem;

    if (data_type_id == static_cast<int>(DataType::FP16)) {
        launch_flash_attention_prefill_fp16(
            static_cast<half *>(output),
            static_cast<const half *>(query),
            static_cast<const half *>(key),
            static_cast<const half *>(value),
            batch_size, q_seq_len, kv_seq_len, num_heads, num_kv_heads,
            head_dim, threads_per_block, scale, shared_mem_size, stream
        );
    } else if (data_type_id == static_cast<int>(DataType::FP8)) {
        launch_flash_attention_prefill_fp8(
            static_cast<__nv_fp8_e4m3 *>(output),
            static_cast<const __nv_fp8_e4m3 *>(query),
            static_cast<const __nv_fp8_e4m3 *>(key),
            static_cast<const __nv_fp8_e4m3 *>(value),
            batch_size, q_seq_len, kv_seq_len, num_heads, num_kv_heads,
            head_dim, threads_per_block, scale, shared_mem_size, stream
        );
    } else if (data_type_id == static_cast<int>(DataType::FP32)) {
        const auto out_ptr = static_cast<float *>(output);
        const auto q_ptr = static_cast<const float *>(query);
        const auto k_ptr = static_cast<const float *>(key);
        const auto v_ptr = static_cast<const float *>(value);

        if (head_dim == 256) {
            const auto k_kernel = flash_attention_scalar_prefill_kernel<256>;
            cudaFuncSetAttribute(k_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
            k_kernel<<<grid, threads_per_block, shared_mem_size, stream>>>(out_ptr, q_ptr, k_ptr, v_ptr, q_seq_len, kv_seq_len, num_heads, num_kv_heads, scale);
        } else if (head_dim == 128) {
            const auto k_kernel = flash_attention_scalar_prefill_kernel<128>;
            cudaFuncSetAttribute(k_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
            k_kernel<<<grid, threads_per_block, shared_mem_size, stream>>>(out_ptr, q_ptr, k_ptr, v_ptr, q_seq_len, kv_seq_len, num_heads, num_kv_heads, scale);
        } else if (head_dim == 64) {
            const auto k_kernel = flash_attention_scalar_prefill_kernel<64>;
            cudaFuncSetAttribute(k_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
            k_kernel<<<grid, threads_per_block, shared_mem_size, stream>>>(out_ptr, q_ptr, k_ptr, v_ptr, q_seq_len, kv_seq_len, num_heads, num_kv_heads, scale);
        }
    }
}
}
