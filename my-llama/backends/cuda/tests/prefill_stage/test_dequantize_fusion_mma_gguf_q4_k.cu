...
TEST_CASE("GgufQ4KGemmTest - TestBF16") {
    GgufQ4KGemmContext ctx;
    const std::vector<__nv_bfloat16> h_input(ctx.M * ctx.K, __float2bfloat16(0.1f));
    std::vector<__nv_bfloat16> h_output(ctx.M * ctx.N);
    ...
    launch_fused_gemm_gguf_q4_k(d_out, d_in, d_w, ctx.M, ctx.N, ctx.K, static_cast<int32_t>(DataType::BF16), nullptr);
    REQUIRE(cudaDeviceSynchronize() == cudaSuccess);

    REQUIRE(cudaMemcpy(h_output.data(), d_out, h_output.size() * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost) == cudaSuccess);

    for (int32_t m = 0; m < ctx.M; ++m) {
        for (int32_t n = 0; n < ctx.N; ++n) {
            float expected = 0.0f;
            for (int32_t k = 0; k < ctx.K; ++k) {
                expected += 0.1f * ctx.h_unpacked_weights[n * ctx.K + k];
            }
            const float actual = __bfloat162float(h_output[m * ctx.N + n]);
            CHECK(actual == doctest::Approx(expected).epsilon(1.0));
        }
    }
    cudaFree(d_out);
    cudaFree(d_in);
...
TEST_CASE("GgufQ4KGemmTest - TestFP8") {
    GgufQ4KGemmContext ctx;
    std::vector<__nv_fp8_e4m3> h_input(ctx.M * ctx.K);
    for (auto &val: h_input) val = static_cast<__nv_fp8_e4m3>(0.25f);
    std::vector<__nv_fp8_e4m3> h_output(ctx.M * ctx.N);

    void *d_out = nullptr;
    void *d_in = nullptr;
    void *d_w = nullptr;
    REQUIRE(cudaMalloc(&d_out, ctx.M * ctx.N * sizeof(__nv_fp8_e4m3)) == cudaSuccess);
    REQUIRE(cudaMalloc(&d_in, ctx.M * ctx.K * sizeof(__nv_fp8_e4m3)) == cudaSuccess);
    REQUIRE(cudaMalloc(&d_w, ctx.h_weights_quant.size() * sizeof(BlockQ4K)) == cudaSuccess);

    REQUIRE(cudaMemcpy(d_in, h_input.data(), h_input.size() * sizeof(__nv_fp8_e4m3), cudaMemcpyHostToDevice) == cudaSuccess);
    REQUIRE(cudaMemcpy(d_w, ctx.h_weights_quant.data(), ctx.h_weights_quant.size() * sizeof(BlockQ4K), cudaMemcpyHostToDevice) == cudaSuccess);

    launch_fused_gemm_gguf_q4_k(d_out, d_in, d_w, ctx.M, ctx.N, ctx.K, static_cast<int32_t>(DataType::FP8), nullptr);
    REQUIRE(cudaDeviceSynchronize() == cudaSuccess);

    REQUIRE(cudaMemcpy(h_output.data(), d_out, h_output.size() * sizeof(__nv_fp8_e4m3), cudaMemcpyDeviceToHost) == cudaSuccess);
    ...
    for (int32_t m = 0; m < ctx.M; ++m) {
        for (int32_t n = 0; n < ctx.N; ++n) {
            float expected = 0.0f;
            for (int32_t k = 0; k < ctx.K; ++k) {
                expected += in_val * ctx.h_unpacked_weights[n * ctx.K + k];
            }
            const float actual = static_cast<float>(h_output[m * ctx.N + n]);
            CHECK(actual == doctest::Approx(expected).epsilon(1.0));
        }
    }
    cudaFree(d_out);
...
TEST_CASE("GgufQ4KGemmTest - TestFP4") {
    GgufQ4KGemmContext ctx;
    const std::vector<uint8_t> h_input(ctx.M * ctx.K / 2, 0x11);
    std::vector<uint8_t> h_output(ctx.M * ctx.N / 2, 0xFF);

    void *d_out = nullptr;
    void *d_in = nullptr;
    void *d_w = nullptr;
    REQUIRE(cudaMalloc(&d_out, (ctx.M * ctx.N / 2) * sizeof(uint8_t)) == cudaSuccess);
    REQUIRE(cudaMalloc(&d_in, (ctx.M * ctx.K / 2) * sizeof(uint8_t)) == cudaSuccess);
    REQUIRE(cudaMalloc(&d_w, ctx.h_weights_quant.size() * sizeof(BlockQ4K)) == cudaSuccess);

    REQUIRE(cudaMemset(d_out, 0, (ctx.M * ctx.N / 2) * sizeof(uint8_t)) == cudaSuccess);
    REQUIRE(cudaMemcpy(d_in, h_input.data(), h_input.size() * sizeof(uint8_t), cudaMemcpyHostToDevice) == cudaSuccess);
    REQUIRE(cudaMemcpy(d_w, ctx.h_weights_quant.data(), ctx.h_weights_quant.size() * sizeof(BlockQ4K), cudaMemcpyHostToDevice) == cudaSuccess);

    launch_fused_gemm_gguf_q4_k(d_out, d_in, d_w, ctx.M, ctx.N, ctx.K, static_cast<int32_t>(DataType::FP4), nullptr);
    REQUIRE(cudaDeviceSynchronize() == cudaSuccess);

    REQUIRE(cudaMemcpy(h_output.data(), d_out, h_output.size() * sizeof(uint8_t), cudaMemcpyDeviceToHost) == cudaSuccess);
    ...
    for (int32_t m = 0; m < ctx.M && !has_error; ++m) {
        for (int32_t n = 0; n < ctx.N && !has_error; ++n) {
            const int32_t global_element_idx = m * ctx.N + n;
            ...
            for (int32_t k = 0; k < ctx.K; ++k) {
                expected_accum += in_val * ctx.h_unpacked_weights[n * ctx.K + k];
            }

            if (std::abs(actual - expected_accum) > 2.5f) {
