    ...
    void SetUp() override {
        ...
        for (int32_t n = 0; n < N; ++n) {
            for (int32_t k = 0; k < K; ++k) {
                ...
                const float d_super = 0.01f * static_cast<float>(sc);
                const float m_super = 0.002f * static_cast<float>(min_sc);
...
TEST_F(GgufQ4KGemmTest, TestFP4) {
    ...
    for (int32_t m = 0; m < M; ++m) {
        for (int32_t n = 0; n < N; ++n) {
            ...
            const int32_t global_byte_stride = N >> 1;
            const int32_t target_byte_idx = m * global_byte_stride + (n >> 1);
            const uint8_t packed_byte = h_output[target_byte_idx];
            const uint8_t nibble = (n & 1) == 0 ? packed_byte & 0x0F : (packed_byte >> 4);
