...
static void run_rmsnorm_benchmark(
    ...
    void *d_out, size_t single_input_bytes, L2CacheFlusher &flusher
) {
    ...
    size_t scales_size_bytes = (data_type == static_cast<int32_t>(DataType::FP4))
...
void run_rmsnorm_benchmarks() {
    ...
    std::vector h_gamma(hidden_size, __float2bfloat16(1.0f));
    DeviceBuffer d_gamma(h_gamma);
    DeviceBuffer<__nv_bfloat16> d_out(total_tokens * hidden_size);

    {
        size_t elements = total_tokens * hidden_size;
        std::vector h_in(elements, __float2bfloat16(0.5f));
    ...
    {
        size_t elements = total_tokens * hidden_size;
        std::vector<uint8_t> h_in(elements, 0x25);
        DeviceBuffer d_in(h_in);
    ...
    {
        size_t elements = total_tokens * (hidden_size / 2);
        std::vector<uint8_t> h_in(elements, 0x12);
        DeviceBuffer d_in(h_in);

        std::vector h_scales(hidden_size / 32, 1.25f);
        DeviceBuffer d_scales(h_scales);
