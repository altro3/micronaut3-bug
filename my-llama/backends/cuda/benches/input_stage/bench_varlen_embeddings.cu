...
static void run_embeddings_benchmark(
    ...
    void *d_out, const size_t single_weight_row_bytes, const L2CacheFlusher &flusher
) {
...
void run_varlen_embeddings_benchmarks() {
    ...
    const DeviceBuffer d_tokens(h_tokens);
    const DeviceBuffer d_seq_offsets(h_seq_offsets);
    const DeviceBuffer d_block_table(h_block_table);
    const DeviceBuffer d_slot_mapping(h_slot_mapping);
    const DeviceBuffer<float> d_weight_scales(vocab_size * (out_features / 32), 0);
    const DeviceBuffer<__nv_bfloat16> d_out(total_tokens * out_features);

    L2CacheFlusher flusher;

    {
        constexpr size_t total_weight_elements = static_cast<size_t>(vocab_size) * out_features;
        const DeviceBuffer<__nv_bfloat16> d_weight(total_weight_elements, 0);
    ...
    {
        constexpr size_t total_weight_elements = static_cast<size_t>(vocab_size) * out_features;
        const DeviceBuffer<uint8_t> d_weight(total_weight_elements, 0);
