...
static size_t get_shmem_size() {
    const size_t shmem_input = TILE_M * TILE_K * sizeof(__nv_bfloat16) + TILE_N * TILE_K * sizeof(__nv_bfloat16);
...
extern "C" void launch_fused_gemm_gguf_q4_k(
    ...
    const size_t shmem_size = get_shmem_size<TILE_M, TILE_N, TILE_K>();
    ...
    switch (type) {
        case DataType::BF16: {
            ...
            run_fused_gemm_gguf_q4_k_bf16(static_cast<cutlass::bfloat16_t *>(output_activations), static_cast<const cutlass::bfloat16_t *>(input_activations), static_cast<const BlockQ4K *>(quantized_weights), batch_size_or_tokens, hidden_units_out, hidden_units_in, grid, block, shmem_size, stream);
            break;
        }
        case DataType::FP8: {
            if (shmem_size >= 48 * 1024) {
            ...
            run_fused_gemm_gguf_q4_k_fp8(static_cast<__nv_fp8_e4m3 *>(output_activations), static_cast<const __nv_fp8_e4m3 *>(input_activations), static_cast<const BlockQ4K *>(quantized_weights), batch_size_or_tokens, hidden_units_out, hidden_units_in, grid, block, shmem_size, stream);
            break;
        }
        case DataType::FP4: {
            if (shmem_size >= 48 * 1024) {
            ...
            run_fused_gemm_gguf_q4_k_fp4(static_cast<__nv_fp4_e2m1 *>(output_activations), static_cast<const __nv_fp4_e2m1 *>(input_activations), static_cast<const BlockQ4K *>(quantized_weights), batch_size_or_tokens, hidden_units_out, hidden_units_in, grid, block, shmem_size, stream);
            break;
        }
    }
}
